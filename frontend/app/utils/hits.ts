...
export async function getTrades(market:string):Promise<trades[]>{
    const res = await axios.get(BASE_URL+`/trades?symbol=${market}`);
    return res.data();
}
...
export async function getticker(market:string):Promise<Ticker>{
    const res = await axios.get(BASE_URL+`/ticker?symbol=${market}`);
    return res.data();
}
export async function getklines (starttime:string,endtime:string,market:string):Promise<KLine[]>{
    const res = await axios.get(BASE_URL+`/klines?symbol=${market}&intercal=1h&startTime=${starttime}&endTime=${endtime}`);
